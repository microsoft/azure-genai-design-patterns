...
def generate_chunks_from_markdown(doc_layout):
	...
	for raw_chunk in raw_chunks:
		...
		for sub_chunk in sub_chunks:
			chunks.append(build_chunk("text", chunk_index, sub_chunk['title'], sub_chunk['content'], doc_layout))
			chunk_index += 1
	return chunks
...
def chunk_document(doc_file_path, doc_layout):
	...
	print(f"Rendering {doc_file_path} as images with {DPI} dpi resolution ...") # TODO: move this part after polygon processing, and skip it if there's no figure in the doc
	...
	for i, page in enumerate(page_polygon_info):
		...
		for j, polygon in enumerate(page_polygon_info[page]['polygons']):
			...
			cv2.imwrite(f"../../data-images/{doc_layout['document_name']}_{page}_{j}.png", cropped_image)
